...
void Circle_face::set_radius(int rr)
{
    ...
    l_eye.set_major(rr/3);
    r_eye.set_minor(rr/3);
    r_eye.set_major(rr/3);
...
Smiley::Smiley(Point p,int rr): Circle_face(p,rr){mouth.set_angles(180,360);}
...
Smiley_with_hat :: Smiley_with_hat(Point p,int rr)
: Smiley(p,rr),rec1(Point(p.x-rr,p.y-rr),rr*2,rr/3),rec2(Point(p.x-(rr/2),p.y-2*rr),rr*(3/2),rr) 
{
    rec1.set_color(Color::invisible);
    rec1.set_fill_color(color());
    rec2.set_color(Color::invisible);
    rec2.set_fill_color(color());
...
Frowny_with_hat :: Frowny_with_hat(Point p,int rr)
: Frowny(p,rr),rec3(Point(p.x-rr,p.y-rr),rr*2,rr/3),rec4(Point(p.x-(rr/2),p.y-2*rr),rr*(3/2),rr) 
{
    rec3.set_color(Color::invisible);
    rec3.set_fill_color(color());
    rec4.set_color(Color::invisible);
    rec4.set_fill_color(color());
